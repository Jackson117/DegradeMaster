...
from degrademaster.pdb_utils import Atom, VOCAB, dist_matrix_from_coords
...
def blocks_to_data(*blocks_list: List[List[Block]]):
    B, A, X, atom_positions, block_lengths, segment_ids, war_e3_ids = [], [], [], [], [], [], []
    for i, blocks in enumerate(blocks_list):
        ...
        if i < 2:
            ...
            raise ValueError('input block numbers must be 4!')
        ...
        war_e3_ids.extend(cur_war_e3_ids)
    ...
    data = {
        ...
        'war_e3_ids': war_e3_ids,        # [Nresidue]
    }
...
class BlockGeoAffDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_file, database=None, dist_th=6, n_cpu=4, suffix=''):
        ...
        if os.path.exists(proc_file):
            print(f'Loading preprocessed data from {proc_file}...')
        ...
        if need_process:
            ...
            print(f'Preprocessed data saved to {proc_file}')
    ...
    def collate_fn(cls, batch):
        keys = ['X', 'B', 'A', 'atom_positions', 'block_lengths']
        ...
        for i, item in enumerate(batch):
            segment_ids = [seg_id + i * 4 for seg_id in item['segment_ids']]
            seg_list.extend(segment_ids)
        res['segment_ids'] = torch.tensor(seg_list, dtype=torch.long)
